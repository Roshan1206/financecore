...
/**
 * Feign request interceptor that automatically adds OAuth2 Bearer token
 * to outgoing requests for inter-service communication.
 *
 * @author Roshan
 */
@Slf4j
public class OAuth2FeignRequestInterceptor implements RequestInterceptor {
    ...
    public void apply(RequestTemplate template) {
        ...
        if (authorizedClient != null) {
            ...
            log.error("Failed to obtained Bearer token for service {} with principal {}", clientRegistrationId, principalName);
        }
    }
}
